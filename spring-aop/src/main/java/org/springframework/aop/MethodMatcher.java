 ...
 * @see ClassFilter
 *
 * MethodMatcher是PointCut的一部分：用来检查目标方法是否适合advice。
 * 一个MethodMatcher可以分为statically或者runtime（dynamically）
 *
 * 静态匹配涉及方法和(可能的)方法属性
 * 动态匹配还可以使用方法参数
 *
 * 如果isRuntime()方法返回false，会进行静态匹配
 * 如果isRuntime()方法返回true，会先执行静态匹配，静态匹配返回true的情况下会执行动态匹配
 */
